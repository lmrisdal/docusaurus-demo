...
| Warning | A failure has happened while executing the CX pipeline, but is non critical and the process is either continued or retried as described on the [retry page](/retry). |
...
| Terminated | The pipeline was terminated by the user. Either via [code components](/Transformation/Code-Components) of via [data collection](/Transformation/Data-Collection).|
...
When the pipeline instance is created within CX upon data entering through an adapter, a context is implicitly created with the message. This context is described on [the Metadata page](/Metadata). Below we describe how you can influence logging by turning metadata on or off.
...
All logging options require a [Security Configuration](/Security/Security-Configurations). Set this up by following the steps described on the Security Configuration page and select the definition as described below.
...
- **Archeo Security Configuration**: The [security configuration](/Security/Security-Configurations) to use for authenticating the request to Archeo.
- **Log Level**: Explained in the [Log Levels section](#log-levels).
- **Contract**: Is explained under the [Contracts section](#contracts).
- **Inbound message type**: Changes the message type for the first success message logged.
- **Outbound message type**: Changes the message type for the last success message logged.
...
As seen above you can exclude the actual message content from all logs. In a lot of cases the [metadata](/Metadata) that is the log event itself is enough to monitor integration flows. Think carefully before you log the actual content.
...
In many cases there is integration being done either before of after a message is passed of to CX for processing. We recommend tying these actions into the CX flow with you own logging, in this way you get a complete picture of the actual actions involved in you integration. This is very easy with Archeo, and should be possible with most third party providers as well. When setting this up you need to use your InterchangeId to pair your internal logs with CX's generated log events. See more about InterchangeId [here](/Core-Concepts).
